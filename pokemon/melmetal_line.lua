...
local melmetal = {
  ...
  loc_vars = function(self, info_queue, card)
    type_tooltip(self, info_queue, card)
    info_queue[#info_queue+1] = {set = 'Other', key = 'designed_by', vars = {"Sonfive"}}
		return {vars = {card.ability.extra.Xmult_multi, (#find_pokemon_type("Metal")*card.ability.extra.Xmult_multi), #find_joker('metalcoat')}}
  end,
  rarity = 4,
  cost = 20,
  stage = "Legendary",
  ...
  calculate = function(self, card, context)
    ...
    if context.repetition and not context.end_of_round and context.cardarea == G.hand and SMODS.has_enhancement(context.other_card, 'm_steel') then
      local rhytriggers = #find_pokemon_type("Metal")
